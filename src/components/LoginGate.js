...
const LoginGate = ({ isOpen, onClose, onGuest }) => {
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      // If backend returns unverified-like error, prompt verify inline
      if ((e.message || '').toLowerCase().includes('verify') || (e.message || '').includes('not_verified')) {
        setMsg('Your email is not verified. Please enter the OTP sent to your email.');
        setShowVerify(true);
        setTab('login');
      } else {
        setMsg(e.message);
  ...
  const doRegister = async () => {
    setLoading(true); setMsg('');
    try {
      await post('/api/auth/register', { username: username.trim(), email: email.trim(), password, requireOtp: true });
      setMsg('Registered. Check your email for a 6-digit OTP.');
      setShowVerify(true);
      setTab('login');
      // Pre-fill email for verification convenience
      if (!emailOrUsername) setEmailOrUsername(email);
  ...
  const doVerify = async () => {
    setLoading(true); setMsg('');
    try {
      const data = await post('/api/auth/verify', { email: (email || emailOrUsername).trim(), otp: otp.trim() });
      localStorage.setItem('studymate_token', data.token);
      localStorage.setItem('studymate_username', data.user?.username || '');
      onClose();
    } catch (e) { setMsg(e.message); } finally { setLoading(false); }
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                {showVerify && (
                  <div className="space-y-3">
                    <div className="flex items-center bg-academic-100 rounded-lg px-3">
                      <Mail className="w-4 h-4 text-academic-500" />
                      <input className="input-field border-0 bg-transparent" placeholder="Email" value={email || emailOrUsername} onChange={(e)=>{ setEmail(e.target.value); if (!emailOrUsername) setEmailOrUsername(e.target.value); }} />
                    </div>
                    <div className="flex items-center bg-academic-100 rounded-lg px-3">
                    ...
                    <button className="btn-primary w-full" disabled={loading} onClick={doVerify}>{loading ? 'Verifying…' : 'Verify OTP'}</button>
                  </div>
                )}
