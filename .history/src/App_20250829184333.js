...
function App() {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem('studymate_username') || '';
  ...
  const sendMessage = async (message) => {
    ...
    if (!token) {
      setChatHistory(prev => [...prev, {
        id: Date.now(),
        role: 'assistant',
        content: 'Please log in to continue. Use the Login button in the top right.',
  ...
  const handleAuthenticated = ({ username: name }) => {
    const storedUser = name || localStorage.getItem('studymate_username') || '';
    setUsername(storedUser);
  };

  return (
    <div className="min-h-screen bg-academic-50 flex flex-col">
      <Navbar onMenuClick={toggleSidebar} onLoginClick={openLogin} onLogoutClick={handleLogout} username={username} />
      
      <div className="flex flex-1 overflow-hidden relative">
        {/* Desktop persistent sidebar */}
        <div className="hidden lg:block w-80 bg-white border-r border-academic-200 flex-shrink-0">
          <Sidebar
      ...
      <AuthModal isOpen={authOpen} onClose={() => setAuthOpen(false)} onAuthenticated={handleAuthenticated} />
    </div>
  );
}
