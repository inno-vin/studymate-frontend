        ...
        <form onSubmit={handleSubmit} className="flex items-end">
          <div className="flex-1 relative">
            <textarea
              ...
              className="input-field resize-none min-h-[44px] leading-normal max-h-40 overflow-y-auto pr-16"
              rows={1}
              disabled={isLoading}
            />
            {/* Helper hint bottom-left */}
            <div className="absolute bottom-2 left-3 text-xs text-academic-400 pointer-events-none">
              Enter to send • Shift+Enter = new line
            </div>
            {/* Send button inside the input, centered vertically */}
            <motion.button
              type="submit"
              aria-label="Send"
              ...
              whileTap={{ scale: 0.95 }}
              className={`absolute top-1/2 -translate-y-1/2 right-2 h-10 w-10 flex items-center justify-center rounded-md transition-all duration-200 ${
