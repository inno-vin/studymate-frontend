...
function App() {
  ...
  const sendMessage = async (message) => {
    ...
    try {
      ...
      // Add PDF files with backend-compatible field name
      uploadedDocs.forEach(doc => {
        // Primary field name expected by backend
        formData.append('files', doc.file);
        // Backward compatibility with older servers that expect 'pdfs'
  ...
  return (
    <div className="min-h-screen bg-academic-50 flex flex-col">
      ...
      <LoginGate isOpen={gateOpen} onClose={() => setGateOpen(false)} onGuest={() => {}} />
    </div>
  );
}
